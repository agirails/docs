...
interface Manifest {
  ...
  divergences: {
    ...
    behavioral_diffs: Array<{ command?: string; ts: string; python: string; notes?: string }>;
  };
}
...
function renderMcpPage(mcp: Manifest['mcp'], generatedAt: string): string {
  ...
  // Per-tool detail with param schemas auto-extracted from Zod source.
  // Each tool gets an anchor matching its name, so an LLM constructing
  // an MCP call can deep-link to the params.
  const allTools = [
    ...mcp.layers.discovery,
    ...mcp.layers.runtime,
    ...mcp.layers.protocol,
  ];
  const toolsWithParams = allTools.filter((t) => t.params !== undefined);
  if (toolsWithParams.length > 0) {
    lines.push('## Per-tool input schemas');
    lines.push('');
    lines.push('Auto-extracted from the Zod schema declarations in `src/tools/layer1-discovery.ts` and `src/tools/layer2-runtime.ts`. Anchors match the tool name; paste e.g. `agirails_request_service` into the page search to jump directly to its params.');
    lines.push('');
    for (const t of allTools.sort((a, b) => a.name.localeCompare(b.name))) {
      if (t.params === undefined) continue;
      lines.push(`### \`${t.name}\` {#${t.name}}`);
      lines.push('');
      const meta: string[] = [`**Layer**: ${t.layer}`];
      if (t.read_only) meta.push('**Read-only**');
      if (t.destructive) meta.push('**Destructive**');
      if (t.input_schema) meta.push(`**Schema**: \`${t.input_schema}\``);
      ...
      for (const p of t.params) {
        ...
        if (p.enum_values && p.enum_values.length > 0) {
          typeCell = `\`enum\` ${p.enum_values.map((v) => `\`'${v}'\``).join(' \\| ')}`;
        }
        const req = p.required ? '✓' : '';
        const desc = p.description ? escapeMd(p.description) : '_(no description)_';
...
function renderCrossSdkDivergencesPage(
  divergences: Manifest['divergences'],
  generatedAt: string,
): string {
  const lines: string[] = [];
  const s = divergences.summary;
  lines.push('---');
  lines.push('slug: /reference/cross-sdk-divergences');
  lines.push('title: "Cross-SDK divergences"');
  lines.push(`description: "Every TypeScript-only symbol, every Python-only symbol, plus curated name and behavioral differences across the two SDKs. Auto-extracted from the truth-ledger manifest: ${s.ts_only_count} TS-only + ${s.python_only_count} Python-only + ${s.name_diffs_count} name diff + ${s.behavioral_diffs_count} behavioral diff."`);
  lines.push('schema_type: APIReference');
  lines.push(`last_verified: ${generatedAt.slice(0, 10)}`);
  lines.push('auto_extracted_source: "static/sdk-manifest.json"');
  lines.push('stability: stable');
  lines.push('last_breaking_change: 2026-05-19');
  lines.push('tags: [reference, cross-sdk, parity, auto-extracted]');
  lines.push('sidebar_position: 8');
  lines.push('---');
  lines.push('');
  lines.push(GENERATED_BANNER);
  lines.push('');
  lines.push('# Cross-SDK divergences');
  lines.push('');
  lines.push(`**TypeScript-only**: ${s.ts_only_count} symbols · **Python-only**: ${s.python_only_count} symbols · **Name diffs**: ${s.name_diffs_count} · **Behavioral diffs**: ${s.behavioral_diffs_count} · **Manifest generated**: ${generatedAt.slice(0, 19).replace('T', ' ')} UTC`);
  lines.push('');
  lines.push('If you use both SDKs (multi-language stack, or porting code between them), this is the single page that tells you what is missing where. The data is the same as per-symbol `Cross-SDK status` columns in [`sdk-js`](/reference/sdk-js) and [`sdk-python`](/reference/sdk-python), but grouped for quick scanning.');
  lines.push('');
  lines.push('Surfaces compared:');
  lines.push('');
  lines.push('- **`sdk_api`**: public SDK exports (classes, functions, types). The largest divergence surface, driven mostly by TS having more type definitions and Python having more storage/circuit-breaker helpers.');
  lines.push('- **`errors`**: error class catalog. Smaller divergence; x402 error subclasses are TS-only because the Python SDK does not yet route x402.');
  lines.push('- **`cli`**: `actp` subcommand surface. Some commands are TS-only (older Commander-side flags) or Python-only (newer Typer-side ergonomics).');
  lines.push('');
  lines.push('Name divergences and behavioral divergences are **curated**, not derived from set diff: they capture cases where the same concept appears under different names (e.g. `DeadlineExpiredError` vs `DeadlinePassedError`) or with different runtime behavior (e.g. CLI flag rejected on one side, silently ignored on the other).');
  lines.push('');

  const renderSetList = (label: string, items: string[]) => {
  ...
  lines.push('## TypeScript-only');
  lines.push('');
  renderSetList('SDK API', divergences.ts_only.sdk_api);
  renderSetList('Errors', divergences.ts_only.errors);
  renderSetList('CLI', divergences.ts_only.cli);

  lines.push('## Python-only');
  lines.push('');
  renderSetList('SDK API', divergences.python_only.sdk_api);
  renderSetList('Errors', divergences.python_only.errors);
  renderSetList('CLI', divergences.python_only.cli);

  if (divergences.name_diffs.length > 0) {
    lines.push('## Name divergences');
    lines.push('');
    lines.push('Same concept, different name in each SDK. Curated, not inferred. If `python_has_alias` is true, the Python SDK re-exports the TS name as an alias so both names resolve in Python.');
    lines.push('');
    lines.push('| Concept | TypeScript | Python | Python alias | Notes |');
    lines.push('|---|---|---|---|---|');
    for (const d of divergences.name_diffs) {
      const alias = (d as { python_has_alias?: boolean }).python_has_alias ? '✓' : '';
      const notes = (d as { notes?: string }).notes ?? '';
      lines.push(`| \`${d.concept}\` | \`${d.ts}\` | \`${d.python}\` | ${alias} | ${escapeMd(notes)} |`);
    }
    lines.push('');
  }

  if (divergences.behavioral_diffs.length > 0) {
    lines.push('## Behavioral divergences');
    lines.push('');
    lines.push('Same surface name, different runtime behavior. These are bugs or pre-parity gaps tracked through to fix; the divergence is documented here so an integrator who hits it knows it is known.');
    lines.push('');
    for (const d of divergences.behavioral_diffs) {
      const command = (d as { command?: string }).command;
      if (command) {
        lines.push(`### \`${command}\``);
      } else {
        lines.push(`### Unnamed behavioral diff`);
      }
      lines.push('');
      lines.push(`- **TypeScript**: ${escapeMd(d.ts)}`);
      lines.push(`- **Python**: ${escapeMd(d.python)}`);
      const notes = (d as { notes?: string }).notes;
      if (notes) {
        lines.push(`- **Notes**: ${escapeMd(notes)}`);
      }
      lines.push('');
    }
  }
  ...
  lines.push('');
  lines.push('- **Porting code between SDKs**: search this page first for the symbol you depend on; if it appears in TS-only or Python-only, you need a fallback or polyfill.');
  lines.push('- **Multi-language stack**: scan the SDK API lists once to internalize the gaps; you will hit them at integration time and want to recognize them.');
  lines.push('- **Filing a parity bug**: confirm the divergence is not already in the name-diffs or behavioral-diffs tables (those are intentional or already tracked).');
  lines.push('');
  lines.push('See also:');
  lines.push('');
  lines.push('- [TypeScript SDK reference](/reference/sdk-js)');
  lines.push('- [Python SDK reference](/reference/sdk-python)');
  lines.push('- [Error reference](/reference/errors): per-code triage');
  lines.push('- [CLI reference](/reference/cli)');
  lines.push('- [Truth-ledger manifest (raw JSON)](/sdk-manifest.json): the source the per-set lists above are extracted from');
  lines.push('');
...
function main(): void {
  ...
  writeFile('reference/cross-sdk-divergences.md', renderCrossSdkDivergencesPage(m.divergences, m._generatedAt));
