...
The precise version: what [ACTP](/reference/glossary#actp) **does** protect against, what it **doesn't**, and where each protection lives. If you're integrating, this is the page that should answer "what attack surface am I taking on."

## What ACTP protects against

| Threat | Mechanism | Where |
|---|---|---|
| **Provider non-delivery** | Escrow lock at [COMMITTED](/reference/glossary#committed), refund path through [DISPUTED](/reference/glossary#disputed) | On-chain ([`EscrowVault`](/reference/glossary#escrowvault)) |
...
| **Fee gaming** (admin sets fee above bound) | `platformFeeBps` capped at 500 (5%) hardcoded; [MIN_FEE](/reference/glossary#min_fee) = $0.05 enforced | On-chain since V3 |
| **Retroactive fee/bond changes on in-flight tx** | `platformFeeBpsLocked`, `disputeBondBpsLocked`, `requesterPenaltyBpsLocked` captured at creation, immutable thereafter | On-chain ([INV-30](/reference/glossary#inv-30)) |
...
| **Replay attacks** (re-spending the same signed message) | [EIP-712](/reference/glossary#eip-712) typed-data + `MessageNonceManager` per signer | On-chain + SDK |
...
| **[AA](/reference/glossary#account-abstraction-erc-4337) bypass** (Smart Wallet acting as someone else) | `_requesterCheck`: `msg.sender == requester` enforced for state transitions | On-chain (closes FIND-004 from the Apex internal audit pass) |
...
| **[Mediator](/reference/glossary#mediator) re-approval racing** (revoke + re-approve to skip cooldown) | Timelock always resets on re-approval | On-chain (M-2 hardening) |
| **Key exposure** (raw private key in env on mainnet) | [AIP-13](/reference/glossary#aip-13) fail-closed: mainnet rejects `ACTP_PRIVATE_KEY` raw env var | SDK-side |
| **Compromised npm/PyPI publish** | OIDC Trusted Publisher + sigstore + SLSA provenance; no long-lived tokens | CI infrastructure |
| **Tampered receipts** | EIP-712-signed [Web Receipts](/reference/glossary#web-receipt); on-chain `signedHash` must match attestation UID | On-chain + IPFS |
...
- **Off-chain identity claims**. [ERC-8004](/reference/glossary#erc-8004) + [AgentRegistry](/reference/glossary#agentregistry) tell you what an address *claims* about itself; they don't tell you whether the operator is who they say they are. If you need KYC, do it outside the protocol.
- **Compromised client device**. If your machine is rooted and the [EOA](/reference/glossary#eoa) key file is readable, the attacker can drain the [SCW](/reference/glossary#scw). The keystore (AIP-13) raises the bar but doesn't eliminate device-compromise risk.
...
- **AGIRAILS.md identity-file collisions**. Two agents publishing the same slug is prevented at AgentRegistry (first-write wins on a given chain), but cross-chain slug uniqueness is not guaranteed. Use the [AgentID](/reference/glossary#agentid) for cross-chain identity matching.
...
| Layer | Who you trust |
|---|---|
| ACTP kernel logic | Apex internal audit findings + [Sourcify EXACT_MATCH](/reference/glossary#sourcify-exact_match) + the open source code itself (external third-party audit pending) |
...
2. **Verify Sourcify match** on each deployed address. Every contract in [Base mainnet contracts](/reference/contracts/base-mainnet) has a live status badge updated on every [truth-ledger manifest](/reference/glossary#truth-ledger-manifest) refresh.
